...
func launchInstance(ctx context.Context, subscriptionID string, cred azcore.TokenCredential, pubKey *string) error {
	...
	resourceGroupParams := armresources.ResourceGroup{
		Location: to.Ptr(configuration.DefaultLocation),
	}
	...
	if !found {
		vnetPollerResp, err := virtualNetworkClient.BeginCreateOrUpdate(
			...
			*resourcegroupResponse.Name,
			configuration.NicResourceGroupName,
		...
		armnetwork.Interface{
			...
			Properties: &armnetwork.InterfacePropertiesFormat{
				...
				IPConfigurations: []*armnetwork.InterfaceIPConfiguration{
					{
						Name: to.Ptr(configuration.NicResourceGroupName),
		...
		Properties: &armcompute.VirtualMachineProperties{
			StorageProfile: &armcompute.StorageProfile{
				ImageReference: &armcompute.ImageReference{
					Offer:     to.Ptr(configuration.ImageOffer),
					Publisher: to.Ptr(configuration.ImagePublisher),
					SKU:       to.Ptr(configuration.ImageSKU),
					Version:   to.Ptr(configuration.ImageVersion),
				},
				OSDisk: &armcompute.OSDisk{
					Name:         to.Ptr(configuration.VmName),
			...
			HardwareProfile: &armcompute.HardwareProfile{
				VMSize: to.Ptr(armcompute.VirtualMachineSizeTypes(configuration.VmSizeAndType)), // VM size include vCPUs,RAM,Data Disks,Temp storage.
			},
			OSProfile: &armcompute.OSProfile{ //
				ComputerName:  to.Ptr(configuration.VmName),
				AdminUsername: to.Ptr(configuration.AdminUsername),
				LinuxConfiguration: &armcompute.LinuxConfiguration{
					DisablePasswordAuthentication: to.Ptr(true),
					SSH: &armcompute.SSHConfiguration{
						PublicKeys: []*armcompute.SSHPublicKey{
							{
								Path:    to.Ptr(fmt.Sprintf("/home/%s/.ssh/authorized_keys", configuration.AdminUsername)),
								KeyData: pubKey,
							},
						},
					},
